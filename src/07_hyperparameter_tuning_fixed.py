"""
Store Sales Forecasting - Hyperparameter Tuning (FIXED)
Version corrigée avec MAPE filtré et GPU support
"""
...
# ============================================
# 4. EVALUATION FUNCTION (FIXED)
# ============================================

def evaluate_predictions(y_true, y_pred):
    """Calcule les métriques - MAPE FILTRÉ"""
    rmse = np.sqrt(mean_squared_error(y_true, y_pred))
    mae = mean_absolute_error(y_true, y_pred)
    
    # MAPE filtré: ignorer les ventes < 1
    mask = y_true >= 1
    if mask.sum() > 0:
...
def cross_validate_model(model, X, y, cv, model_name):
    ...
    for fold, (train_idx, val_idx) in enumerate(cv.split(X), 1):
        ...
        rmse, mae, mape = evaluate_predictions(y_val.values, y_pred)
...
# ============================================
# 5. XGBOOST TUNING (avec GPU si disponible)
# ============================================
print("\n" + "="*70)
...
try:
    ...
    try:
        ...
        print(f"   🖥️  Device: {'GPU (NVIDIA)' if gpu_available else 'CPU'}")
...
if xgb_success:
    ...
    axes[0].set_title('XGBoost Tuning - RMSE', fontweight='bold')
    axes[0].grid(alpha=0.3, axis='x')
    
    axes[1].barh(xgb_df['name'], xgb_df['mae'], color='coral')
    axes[1].set_xlabel('MAE')
    axes[1].set_title('XGBoost Tuning - MAE', fontweight='bold')
    axes[1].grid(alpha=0.3, axis='x')
    
    axes[2].barh(xgb_df['name'], xgb_df['mape'], color='green')
    axes[2].set_xlabel('MAPE (%)')
    axes[2].set_title('XGBoost Tuning - MAPE', fontweight='bold')
    axes[2].grid(alpha=0.3, axis='x')
    
    plt.tight_layout()
    plt.savefig(REPORTS_PATH / '15_xgboost_tuning_fixed.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("   ✅ 15_xgboost_tuning_fixed.png")
...
if lgb_success:
    ...
    axes[0].set_xlabel('RMSE')
    axes[0].set_title('LightGBM Tuning - RMSE', fontweight='bold')
    axes[0].grid(alpha=0.3, axis='x')
    
    axes[1].barh(lgb_df['name'], lgb_df['mae'], color='coral')
    axes[1].set_xlabel('MAE')
    axes[1].set_title('LightGBM Tuning - MAE', fontweight='bold')
    axes[1].grid(alpha=0.3, axis='x')
    
    axes[2].barh(lgb_df['name'], lgb_df['mape'], color='green')
    axes[2].set_xlabel('MAPE (%)')
    axes[2].set_title('LightGBM Tuning - MAPE', fontweight='bold')
    axes[2].grid(alpha=0.3, axis='x')
    
    plt.tight_layout()
    plt.savefig(REPORTS_PATH / '16_lightgbm_tuning_fixed.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("   ✅ 16_lightgbm_tuning_fixed.png")
