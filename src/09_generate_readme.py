"""
Génère un README.md professionnel pour le projet
"""
...
results = pd.read_csv(MODELS_PATH / 'all_models_results.csv')
best_model = results.iloc[0]

readme = f"""# 🛒 Store Sales Time Series Forecasting
...
- 🎯 **{best_model['mape']:.2f}% MAPE** - Performance du meilleur modèle
...
|--------|------|-----|------|-------|
| **{best_model['model']}** | **{best_model['rmse']:.2f}** | **{best_model['mae']:.2f}** | **{best_model['mape']:.2f}%** | **< 1s** |
"""

# Ajouter les autres modèles
for idx, row in results.iterrows():
    if idx > 0:  # Skip best model already shown
        readme += f"| {row['model']} | {row['rmse']:.2f} | {row['mae']:.2f} | {row['mape']:.2f}% | - |\n"
...
- **Impact promotions** : +{((results[results['model']=='Linear Regression'].iloc[0]['mape'] - best_model['mape'])/results[results['model']=='Linear Regression'].iloc[0]['mape']*100):.1f}% de performance avec features avancées
- **Corrélation pétrole** : -0.705 (économie dépendante du pétrole)
- **Amélioration baseline** : {((results[results['model']=='Linear Regression'].iloc[0]['rmse'] - best_model['rmse'])/results[results['model']=='Linear Regression'].iloc[0]['rmse']*100):.1f}% par rapport à la régression linéaire
